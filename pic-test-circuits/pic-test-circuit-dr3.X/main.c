...
unsigned char front_half_period;
...
void interrupt_handler()
{
    ...
    if (RBIF == 1){ // Check for interrupt
        ...
        if (RB0 == 0)
        {
            // Falling edge: save duration of first part of period
            front_half_period = half_period;
        }
        else
        {
            // Rising edge: validate duty cycle
            
            // Accept duty cycles of 35% 
            //                    to 70%
            
            period = front_half_period + half_period;
            
            low_thresh = (period * 7) / 20;
            high_thresh = (period * 7) / 10;
            
            RB1 = ((low_thresh <= front_half_period) && (front_half_period <= high_thresh));
        }
    ...
    if (CCP1IF == 1){    
        // Timer overflow is an invalid clock (too slow to measure)
        CCPR1 = 0;
